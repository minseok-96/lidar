...
class cloud:
	...
	def __init__(self):
		...
		self.point_pub = rospy.Publisher("/scan_",PointCloud,queue_size=10)		
	...
	# groupPoints
		for i in range(0,len(_points)-1):
			...
			#Group(all of Points to PointCloud)		
			#print("i1:{}".format(i))
			# po=Point32()
			# po.x= _points[i][0]
			# po.y=_points[i][1]
			...
			if _distance < self._hypothesis(_dgroup,_dp,_ri):
				print("_distance,_hypo: {0}, {1}".format(_distance,self._hypothesis(_dgroup,_dp,_ri)))
				# insert point to group
				_groupPoints.append(_points[i])

				# last groupPoints to groupPoints_new			
				if i == len(_points)-2:
					if len(_groupPoints) > 10:
						...
						for j in range(0,len(_groupPoints_new[group_num])):
							...
							pc.points.append(po)
						# marker middlepoint
						mp=self.middlePoint(_groupPoints_new[group_num])
